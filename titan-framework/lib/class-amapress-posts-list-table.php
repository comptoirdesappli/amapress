...
if ( ! class_exists( 'Amapress_List_Table' ) ) {
	require_once( 'class-amapress-list-table.php' );
}
 ...
 * @see WP_List_Table
 */
class Amapress_Posts_List_Table extends Amapress_List_Table {
	 ...
	 */
	public function __construct( $prefix, $post_type, $parent_metabox_id, $parent_post, $query, $args = array() ) {
		global $wpdb;

		$this->parent_post = $parent_post;
		$this->query       = $query;

		parent::__construct( $prefix, $post_type, $parent_metabox_id, array(
			'plural' => 'posts',
		...
		$this->user_posts_count = intval( $wpdb->get_var( $wpdb->prepare( "
			...
			AND post_author = %d
		", $post_type, amapress_current_user_id() ) ) );

		if ( $this->user_posts_count && ! current_user_can( $post_type_object->cap->edit_others_posts ) && ( $this->has_this_table_empty_query_arg( 'post_status' ) ) && ( $this->has_this_table_empty_query_arg( 'all_posts' ) ) && ( $this->has_this_table_empty_query_arg( 'author' ) ) && ( $this->has_this_table_empty_query_arg( 'show_sticky' ) ) ) {
			$this->set_this_table_query_arg( 'author', amapress_current_user_id() );
		}
	...
	public function prepare_items() {
		global $per_page, $mode, $avail_post_stati;

		$this->set_hierarchical_display( is_post_type_hierarchical( $this->post_type ) && 'menu_order title' === $this->query->query['orderby'] );
		...
		} elseif ( $this->query->found_posts || $this->get_pagenum() === 1 ) {
			$total_items = $this->query->found_posts;
		} else {
			$post_counts = (array) wp_count_posts( $post_type, 'readable' );

			if ( ( $this->has_this_table_query_arg( 'post_status' ) ) && ! empty( $avail_post_stati ) && in_array( $this->get_this_table_query_arg( 'post_status' ), $avail_post_stati ) ) {
				$total_items = $post_counts[ $this->get_this_table_query_arg( 'post_status' ) ];
			} elseif ( ( $this->has_this_table_query_arg( 'show_sticky' ) ) && $this->get_this_table_query_arg( 'show_sticky' ) ) {
				$total_items = $this->sticky_posts_count;
			} elseif ( ( $this->has_this_table_query_arg( 'author' ) ) && $this->get_this_table_query_arg( 'author' ) == amapress_current_user_id() ) {
		...
		if ( ! ( $this->has_this_table_empty_query_arg( 'mode' ) ) ) {
			$mode = $this->get_this_table_query_arg( 'mode' ) === 'excerpt' ? 'excerpt' : 'list';
			set_user_setting( 'posts_list_mode', $mode );
		} else {
			$mode = get_user_setting( 'posts_list_mode', 'list' );
		}

		$this->is_trash = ( $this->has_this_table_query_arg( 'post_status' ) ) && $this->get_this_table_query_arg( 'post_status' ) === 'trash';
	...
	protected function is_base_request() {
		$vars = $_GET;
		unset( $vars[ $this->prefix . 'paged' ] );

		if ( empty( $vars ) ) {
			return true;
		} elseif ( 1 === count( $vars ) && ! empty( $vars[ $this->prefix . 'post_type' ] ) ) {
			return $this->post_type === $vars[ $this->prefix . 'post_type' ];
		}

		return 1 === count( $vars ) && ! empty( $vars[ $this->prefix . 'mode' ] );
	}
	...
	protected function get_edit_link( $args, $label, $class = '' ) {
		$url = $this->add_this_table_query_arg( $args );
		$url = $this->add_this_table_query_arg( 'parent_post', $this->parent_post, $url );
	...
	protected function get_views() {
		...
		if ( $this->user_posts_count && $this->user_posts_count !== $total_posts ) {
			if ( ( $this->has_this_table_query_arg( 'author' ) ) && ( $this->get_this_table_query_arg( 'author' ) == $current_user_id ) ) {
				$class = 'current';
			}
		...
		if ( empty( $class ) && ( $this->is_base_request() || ( $this->has_this_table_query_arg( 'all_posts' ) ) ) ) {
			$class = 'current';
		}
		...
		foreach ( get_post_stati( array( 'show_in_admin_status_list' => true ), 'objects' ) as $status ) {
			...
			//TODO : $avail_post_stati ????
			if ( ! empty( $avail_post_stati ) && ! in_array( $status_name, $avail_post_stati ) || empty( $num_posts->$status_name ) ) {
				continue;
			}

			if ( ( $this->has_this_table_query_arg( 'post_status' ) ) && $status_name === $this->get_this_table_query_arg( 'post_status' ) ) {
				$class = 'current';
			}
		...
		if ( ! empty( $this->sticky_posts_count ) ) {
			$class = ! ( $this->has_this_table_empty_query_arg( 'show_sticky' ) ) ? 'current' : '';
...
//			}
	...
	public function current_action() {
		if ( ( $this->has_this_table_query_arg( 'delete_all' ) ) || ( $this->has_this_table_query_arg( 'delete_all2' ) ) ) {
			return 'delete_all';
		}
	...
	protected function get_table_classes() {
		return array(
			...
			'fixed',
			'striped',
			is_post_type_hierarchical( $this->post_type ) ? 'pages' : 'posts'
		);
	}
	...
	public function get_columns() {
		...
		$post_status = ! ( $this->has_this_table_empty_query_arg( 'post_status' ) ) ? $this->get_this_table_query_arg( 'post_status' ) : 'all';
	...
	private function _display_rows_hierarchical( $pages, $pagenum = 1, $per_page = 20 ) {
		 ...
		 */
		if ( ( $this->has_this_table_empty_query_arg( 's' ) ) ) {
		...
		if ( ! isset( $GLOBALS[ $this->prefix . 'post' ] ) ) {
			$GLOBALS[ $this->prefix . 'post' ] = reset( $ids );
		}
	...
	public function single_row( $post, $level = 0 ) {
		...
		$GLOBALS[ $this->prefix . 'post' ] = $post;
		setup_postdata( $post );

		$classes = 'iedit author-' . ( amapress_current_user_id() == $post->post_author ? 'self' : 'other' );
		...
		<?php
		$GLOBALS[ $this->prefix . 'post' ] = $global_post;
	}
            ...
            <table style="display: none">
				...
				while ( $bulk < 2 ) { ?>
                    ...
                    <tr id="<?php echo $bulk ? 'bulk-edit' : 'inline-edit'; ?>"
                        class="inline-edit-row inline-edit-row-<?php echo "$hclass inline-edit-" . $this->post_type;
					    echo $bulk ? " bulk-edit-row bulk-edit-row-$hclass bulk-edit-{$this->post_type}" : " quick-edit-row quick-edit-row-$hclass inline-edit-{$this->post_type}";
					    ?>" style="display: none">
                        <td colspan="<?php echo $this->get_column_count(); ?>" class="colspanchange">
                            ...
                            <fieldset class="inline-edit-col-right">
                                <div class="inline-edit-col">
									...
									<?php if ( post_type_supports( $this->post_type, 'comments' ) || post_type_supports( $this->post_type, 'trackbacks' ) ) :
										if ( $bulk ) : ?>
                                            ...
                                            <div class="inline-edit-group wp-clearfix">
												...
												<?php endif;
												if ( post_type_supports( $this->post_type, 'trackbacks' ) ) : ?>
									...
									if ( $bulk && current_theme_supports( 'post-formats' ) && post_type_supports( $this->post_type, 'post-formats' ) ) {
