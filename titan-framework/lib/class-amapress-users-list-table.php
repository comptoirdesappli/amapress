 ...
 * @see WP_List_Table
 */
class Amapress_Users_List_Table extends Amapress_List_Table {
	...
	public function __construct($prefix, $post_type, $parent_metabox_id, $parent_post, $query, $args = array() ) {
		...
		if ( $this->is_site_users )
			{$this->site_id = ( $this->has_this_table_query_arg('id') ) ? intval( $this->get_this_table_query_arg('id') ) : 0;}
	}
	...
	public function prepare_items() {
		...
		$usersearch = ( $this->has_this_table_query_arg('s') ) ? wp_unslash( trim( $this->get_this_table_query_arg('s') ) ) : '';

		$role = ( $this->has_this_table_query_arg('role') ) ? $this->get_this_table_query_arg('role') : '';
		...
		if ( ( $this->has_this_table_query_arg('orderby') ) )
			{$args['orderby'] = $this->get_this_table_query_arg('orderby');}

		if ( ( $this->has_this_table_query_arg('order') ) )
			{$args['order'] = $this->get_this_table_query_arg('order');}
	...
	protected function get_views() {
		...
		foreach ( $wp_roles->get_names() as $this_role => $name ) {
			...
			$role_links[$this_role] = "<a href='" . esc_url( $this->add_this_table_query_arg( 'role', $this_role, $url ) ) . "'$class>$name</a>";
		}
		...
		if ( ! empty( $avail_roles['none' ] ) ) {
			...
			$role_links['none'] = "<a href='" . esc_url( $this->add_this_table_query_arg( 'role', 'none', $url ) ) . "'$class>$name</a>";
	...
	public function current_action() {
		if ( ( $this->has_this_table_query_arg('changeit') ) &&
			( ! ( $this->has_this_table_empty_query_arg('new_role') ) || ! ( $this->has_this_table_empty_query_arg('new_role2') ) ) ) {
			return 'promote';
		}
	...
	public function single_row( $user_object, $style = '', $role = '', $numposts = 0 ) {
		...
		if ( current_user_can( 'list_users' ) ) {
			...
			$edit_link = esc_url( $this->add_this_table_query_arg( 'wp_http_referer', urlencode( wp_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user_object->ID ) ) );
			...
			if ( !is_multisite() && amapress_current_user_id() != $user_object->ID && current_user_can( 'delete_user', $user_object->ID ) )
				{$actions['delete'] = "<a class='submitdelete' href='" . wp_nonce_url( "users.php?action=delete&amp;user=$user_object->ID", 'bulk-users' ) . "'>" . __( 'Delete' ) . "</a>";}
			if ( is_multisite() && amapress_current_user_id() != $user_object->ID && current_user_can( 'remove_user', $user_object->ID ) )
