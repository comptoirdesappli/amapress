...
//			[ $this->responsable_amap, 'amapress_lieu=' . $this->lieu_1 . '&post_type=' . Amapres::INTERNAL_POST_TYPE, [] ],
	...
	function testQuery( $user_id, $query, $expected_posts ) {
		...
		if ( ! is_int( $expected_posts ) ) {
			$this->assertCount( $expected_posts, $post_ids, 'Arrays are not equal : ' . $this->supposedPostResultSet( $post_ids ) );
		} else {
			$this->assertEquals( $expected_posts, $post_ids, 'Arrays are not equal : ' . count( $post_ids ) );
		}
	}
	...
	function testUsers( $user_id, $query, $expected_users ) {
		...
		if ( is_int( $expected_users ) ) {
			$this->assertCount( $expected_users, $user_ids, 'Arrays are not equal : ' . $this->supposedUsersResultSet( $user_ids ) );
		} else {
			$this->assertEquals( $expected_users, $user_ids, 'Arrays are not equal : ' . count( $user_ids ) );
		}
	}

	private function supposedPostResultSet( $post_ids ) {
		return implode( ',',
			array_map(
				function ( $p ) {
					return "\$this->posts['$p']";
	...
	private function supposedUsersResultSet( $user_ids ) {
		return implode( ',',
			array_map(
				function ( $p ) {
					return "\$this->users['$p']";
