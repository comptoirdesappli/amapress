	...
	*/
	...
	protected function initImapStream() {
		...
		return $this->imap( 'open', [
			...
			$this->imapRetriesNum,
			$this->imapParams
		], false, ConnectionException::class );
	}
	...
	public function renameMailbox( $oldName, $newName ) {
		$this->imap( 'renamemailbox', [
			$this->imapPath . $this->getPathDelimiter() . $oldName,
			$this->imapPath . $this->getPathDelimiter() . $newName
		] );
	}
	...
	public function getMailsInfo( array $mailsIds ) {
		$mails = $this->imap( 'fetch_overview', [
			implode( ',', $mailsIds ),
	...
	public function getMail( $mailId, $markAsSeen = true ) {
		...
		$mailStructure = $this->imap( 'fetchstructure', [
			$mailId,
			( SE_UID == $this->imapSearchOption ) ? FT_UID : 0
		] );
	...
	protected function initMailPart( IncomingMail $mail, $partStructure, $partNum, $markAsSeen = true, $emlParse = false ) {
		...
		if ( $this->getAttachmentsIgnore() &&
		     ( TYPEMULTIPART !== $partStructure->type &&
		       ( TYPETEXT !== $partStructure->type || ! \in_array( strtolower( $partStructure->subtype ), [
				       'plain',
				       'html'
			       ] ) ) ) ) {
			return false;
