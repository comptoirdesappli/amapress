...
<html>
				...
				if ( isset( $corr[ $y ][ $x ] ) && is_array( $corr[ $y ][ $x ] ) ) {
					...
					if ( $corr[ $y ][ $x ][3] == 1 ) {
						...
						for ( $j = 1; $j < $corr[ $y ][ $x ][3]; $j ++ ) {
							...
							$ty = $y + $j;
							for (
								true; isset( $corr[ $ty ][ $tx ] ) && ! is_array( $corr[ $ty ][ $tx ] );
								$tx ++
							) {
		...
		if ( $this->_subPart ) {
			...
			for ( $j = 0; $j < $rowspan; $j ++ ) {
				for ( $i = 0; $i < $colspan; $i ++ ) {
					...
					[ self::$_tables[ $param['num'] ]['corr_x'] + $i ] = ( $i + $j > 0 ) ? '' : array(
						$x,
						$y,
						$colspan,
						$rowspan
	...
	protected function _tag_close_TD( $param ) {
		...
		if ( $this->_subPart ) {
			...
			self::$_tables[ $param['num'] ]['cases'][ self::$_tables[ $param['num'] ]['tr_curr'] - 1 ][ self::$_tables[ $param['num'] ]['td_curr'] - 1 ]['w'] = max( array(
				$w0,
				$w2
			) );
			self::$_tables[ $param['num'] ]['cases'][ self::$_tables[ $param['num'] ]['tr_curr'] - 1 ][ self::$_tables[ $param['num'] ]['td_curr'] - 1 ]['h'] = max( array(
				$h0,
				$h2
			) );
