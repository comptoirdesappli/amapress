...
abstract class AbstractVariableRestrictionsSniff extends Sniff {
	...
	public function process_token( $stackPtr ) {
		...
		if ( \in_array( $token['code'], array(
			\T_OBJECT_OPERATOR,
			\T_DOUBLE_COLON
		), true ) ) { // This only works for object vars and array members.
		...
		foreach ( $this->groups_cache as $groupName => $group ) {
			...
			if ( \in_array( $token['code'], array(
				...
				$var   = implode( '', array(
					$this->tokens[ $owner ]['content'],
					$token['content'],
					$this->tokens[ $child ]['content']
				) );
			...
			if ( \in_array( $token['code'], array(
					...
					\T_DOUBLE_QUOTED_STRING,
					\T_HEREDOC
				), true ) && ! empty( $group['array_members'] ) ) {
				// Array members.
