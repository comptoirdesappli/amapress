...
//        foreach ($user_lieux as $lieu_id) {
		...
		foreach ( $dates as $date => $date_dists ) {
			...
			if ( ! empty( $subst_lieux ) ) {
				...
				$ret         .= '<p class="inscr-list-lieux-substitution">' . __( 'exceptionnellement à ', 'amapress' ) . esc_html( $subst_lieux ) . '</p>';
			}
			$ret .= '</th>';
			if ( $for_emargement ) {
...
//                $ret .= '<td>';
				...
				if ( ! $for_emargement && ! $for_pdf && $has_slots ) {
					...
					data-dist="' . $dist->ID . '" data-slot="val:#' . $affect_slot_id . '">' . __( 'Réserver', 'amapress' ) . '</button>';
							...
							$ret .= '<span style="color: orange">' . __( 'clos', 'amapress' ) . '</span>';
						}
					}
					$ret .= "</td>";
				}
...
<button type="button" class="' . $btn_class . ' dist-inscrire-button" data-confirm="' . esc_attr__( 'Etes-vous sûr de vouloir inscrire cet amapien comme gardien de panier ?', 'amapress' ) . '" data-message="val:#garde-msg-' . $dist->ID . '" data-gardien="T" data-dist="' . $dist->ID . '">' . __( 'Inscrire', 'amapress' ) . '</button>
					...
					if ( $allow_gardiens_comments ) {
						$inscr_self = '<textarea id="garde-msg-' . $dist->ID . '" rows="2" style="display: block; width: 100%; padding: 0" placeholder="' . esc_attr__( 'Message', 'amapress' ) . '"></textarea>';
					} else {
						$inscr_self = '';
					}
					...
					if ( ! $for_pdf ) {
						...
						if ( empty( $dist->getGardiensIds() ) ) {
							$info = '<span class=\'distrib-resp-missing\'>' . __( '0 gardien', 'amapress' ) . '</span>';
						} else {
							$info .= '<a href="' . $dist->getPermalink() . '" target="_blank">' . count( $dist->getGardiensIds() ) . __( ' gardien(s)', 'amapress' ) . '</a>';
						}
					}
					if ( $is_user_part_of ) {
...
<button type="button" class="' . $btn_class . ' dist-inscrire-button" data-confirm="' . esc_attr__( 'Etes-vous sûr de vouloir inscrire ce co-adhérent ?', 'amapress' ) . '" data-role="' . $resp_idx . '" data-dist="' . $dist->ID . '">' . __( 'Inscrire', 'amapress' ) . '</button>
							...
							if ( ! $for_pdf ) {
								if ( ( $has_role_names || ! $added_inscr_button || $allow_multi_inscription ) && ( ! $is_resp || $allow_multi_inscription ) && $can_subscribe ) {
									...
									$missing = '<span class=\'distrib-resp-missing\'>' . __( 'manquant', 'amapress' ) . '</span>';
								}
							}
							if ( $is_user_part_of ) {
...
function amapress_next_distrib_shortcode( $atts, $content = null, $tag = null ) {
	...
	switch ( $tag ) {
		...
		case 'next-distrib-deliv-paniers':
			foreach ( $next_distribs as $dist ) {
				/** @var AmapressDistribution $dist */
				$content .= amapress_get_panel_start_no_esc( Amapress::makeLink( $dist->getPermalink(), $dist->getTitle(), true, true ) );
				...
				foreach ( $dist->getContrats() as $contrat_instance ) {
					...
					$content .= amapress_get_panel_start_no_esc( Amapress::makeLink( $contrat_model->getPermalink(), $contrat_instance->getModelTitleWithSubName(), true, true ), $icon,
						'amap-panel-dist amap-panel-dist-' . $dist->getLieuId() . ' amap-panel-dist-panier amap-panel-dist-panier-' . $contrat_model->ID );
					$content .= AmapressPaniers::getPanierContentHtml( $panier->ID, $dist->getLieuId() );
