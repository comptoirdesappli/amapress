...
function amapress_get_contrat_quantite_datatable(
	...
	foreach ( $quants as $quant ) {
		...
		if ( count( $lieux ) > 1 ) {
			foreach ( $lieux as $lieu ) {
				...
				if ( empty( $lieu_quant_adh_count ) ) {
					...
					$row["lieu_{$lieu->ID}"]     = ( $show_adherents && $lieu_quant_count != $lieu_quant_adh_count ?
							"$lieu_quant_adh_count adhérents ; " : '' ) . "$lieu_quant_count $quant_title";
					$row["lieu_{$lieu->ID}_txt"] = "$lieu_quant_count $quant_title";
				} else {
					$row["lieu_{$lieu->ID}"]     = ( $show_adherents && $lieu_quant_count != $lieu_quant_adh_count ?
							"$lieu_quant_adh_count adhérents ; " : '' ) . "$lieu_quant_count $quant_title <i>équivalent quantité : $lieu_quant_sum</i>";
					$row["lieu_{$lieu->ID}_txt"] = "$lieu_quant_count $quant_title";
				}
			}
		}
		...
		if ( empty( $all_quant_adh_count ) ) {
			...
			$row['all']     = ( $show_adherents && $all_quant_count != $all_quant_adh_count ? "$all_quant_adh_count adhérents ; " : '' ) .
			                  "$all_quant_count $quant_title";
			$row['all_txt'] = "$all_quant_count $quant_title";
		} else {
			$row['all']     = ( $show_adherents && $all_quant_count != $all_quant_adh_count ? "$all_quant_adh_count adhérents ; " : '' ) .
			                  "$all_quant_count $quant_title <i>équivalent quantité : $all_quant_sum</i>";
			$row['all_txt'] = "$all_quant_count $quant_title";
		}
		$data[] = $row;
	}
