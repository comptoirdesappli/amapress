		...
		if ( empty( $user_id ) || $user_id <= 0 ) {
			...
			if ( $adhesions ) {
				...
				$ret[] = new Amapress_EventEntry( array(
					...
					'alt'      => 'Vous allez être encaissé ' . ( 'chq' == $this->getType() ? ' du chèque numéro ' . $num : ( 'esp' == $this->getType() ? ' des espèces remises ' : ' du virement ' ) ) . ' d\'un montante de ' . $price . '€ à la date du ' . date_i18n( 'd/m/Y', $date ),
					'href'     => '/mes-adhesions'
				) );
			}
		}
